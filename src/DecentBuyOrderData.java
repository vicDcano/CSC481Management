...
public class DecentBuyOrderData {
    ...
    public void addOrder(Connection dbConn) throws SQLException {
        // select a random item from the inventory where quantity > 0
        String selectSQL = "SELECT idProducts AS id, ProductsName AS product_name, ProductsStock AS quantity FROM Products WHERE ProductsStock > 0 ORDER BY RAND() LIMIT 1";
        ...
        if (rs.next()) {
            ...
            String insertOrderSQL = "INSERT INTO PendingOrders (product_name, quantity) VALUES ('" + itemName + "', 1)";
    ...
    public void loadPendingOrdersData(Connection dbConn, JTable table) throws SQLException {
        String selectSQL = "SELECT order_id, first_name, last_name, product_name, quantity, price, status FROM PendingOrders";
        ResultSet rs = dbConn.createStatement().executeQuery(selectSQL);
    ...
    {
        String selectSQL = "SELECT order_id, first_name, last_name, product_name, quantity, price, status FROM CanceledOrders";
        ResultSet rs = dbConn.createStatement().executeQuery(selectSQL);
    ...
    {
        String selectSQL = "SELECT order_id, first_name, last_name, product_name, quantity, price, status FROM CompletedOrders";
        ResultSet rs = dbConn.createStatement().executeQuery(selectSQL);
        ...
        for (int row = 0; row < model.getRowCount(); row++) {
            ...
            try (PreparedStatement checkStmt = dbConn.prepareStatement(checkSQL)) {
                ...
                if (exists > 0) { // Order exists, so update it
                    String updateSQL = "UPDATE PendingOrders SET first_name = ?, last_name = ?, product_name = ?, quantity = ?, price = ?, status = ? WHERE order_id = ?";
                    ...
                    String insertSQL = "INSERT INTO PendingOrders (first_name, last_name, product_name, quantity, price, status) VALUES (?, ?, ?, ?, ?, ?)";
